...
class DbManage {
	 ...
	 * @return boolean
	 */
	private function _import2($sqlfile) {
		// sql文件包含的sql语句数组
		$sqls = array ();
		$f = fopen ( $sqlfile, "rb" );
		// 创建表缓冲变量
	...
	private function _import($sqlfile) {
		...
		while ( ! feof ( $f ) ) {
			...
			//如果上一句也是INSERT INTO `xxx` VALUES(，则可以合并
			$expstr='` VALUES(';
			$linearr=explode($expstr, $line);
			$lastarr=explode($expstr, $lastline);
			
			$insertstr=$linearr[0].'` VALUES';
			if($linearr[0]===$lastarr[0]){
				//合并
				if(!$lines){
					$lines = $line;
				}else{
					$lines=rtrim($lines,";\n");
					$lines.=','.str_replace($insertstr,'',$line);
				}
			}else{
				// 执行sql语句
				$this->_insert_into ( $line );
